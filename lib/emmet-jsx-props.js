...
export default {
  ...
  activate(state) {
    if (atom.packages.isPackageLoaded('emmet')) {
      ...
      const reBrackets = /\b(\w+)=[{\(](\([ ,{}\d\w]*\) ?=> ?[- _+*{};\(\)\.\w\d]*)[}\)]/;
      const reBackticks = /\b(\w+)=(`[- _=${}\w\d\.\(\)]+`)/;
      const reBackticksQuotes = /\b(\w+)=["'](`[- _=${}\w\d\.\(\)]+`)["']/;
      const reBackticksQuotesG = /\b(\w+)=["'](`[- _=${}\w\d\.\(\)]+`)["']/g;
      const reLambdaQuotes = /\b(\w+)=["'](\([ ,{}\d\w]*\) ?=> ?[- _+*{};\(\)\.\w\d]*)["']/;
      const reQuotesG = /\b(\w+)="(\([ ,{}\d\w]*\) ?=> ?[- _+*{};\(\)\.\w\d]*)"/g;
      ...
      const escapeWithQuotes = (tree) => {
        if (typeof tree !== undefined) {
          ...
          // We are dealing with a lambda function or backticks expression, so update the attributes
      ...
      const transform = (item) => {
        if (typeof item !== undefined) {
          ...
          if (transformBackticks)
            item.start = item.start.replace(reBackticksQuotesG, replacementString);
      ...
      const escapeWithQuotesFilter = (tree) => {
        if (typeof tree !== undefined) {
      ...
      filters.add("jsx-props-2-escapes", escapeWithQuotesFilter);
