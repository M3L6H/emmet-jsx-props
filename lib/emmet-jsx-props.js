      ...
      const escapeLambda = (tree) => {
        if (typeof tree !== undefined) {
          if (expandLambdas) {
            ...
            const result = abrv.match(reQuotes);
            ...
            if (value.includes("=>")) {
              // tree[0]._attributes = [{ name: name, value: value }];
              // Add the corrected lambda function attribute
              const index = tree[0]._attributes.findIndex((elt) => (elt.name === name));
              tree[0]._attributes[index] = value;

              // Remove the fake attributes added by emmet
